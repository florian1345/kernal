...
fn assert_panics_with_message_matching_predicate<R, F, S, P>(
    ...
    P: FnOnce(&str) -> bool,
{
    let failure = Failure::from_expression(&assert_that.expression).expected_it(expected_it);
    ...
    match catch_unwind(assert_that.data).map_err(to_message) {
        ...
        Err(Some(msg)) if !predicate(&msg) => failure
            .but_it(format!("panicked with message <{}>", &msg))
            .fail(),
        _ => {},
    }
}
...
impl<R, F: FnOnce() -> R + UnwindSafe> PanicAssertions for AssertThat<F> {
    ...
    fn does_not_panic(self) {
        if let Err(error) = catch_unwind(self.data) {
            let failure = Failure::from_expression(&self.expression).expected_it("not to panic");
    ...
    fn panics_with_message<M: Borrow<str>>(self, expected_message: M) {
        ...
        assert_panics_with_message_matching_predicate(self, expected_it, |msg| {
    ...
    fn panics_with_message_containing<M: Borrow<str>>(self, expected_message_part: M) {
        ...
        let expected_it = format!(
            "to panic with message containing <{}>",
            expected_message_part
        );

        assert_panics_with_message_matching_predicate(self, expected_it, |msg| {
            msg.contains(expected_message_part)
        });
    }
    ...
    fn panics_with_message_matching<P: FnOnce(&str) -> bool>(self, predicate: P) {
        assert_panics_with_message_matching_predicate(
            self,
            "to panic with message matching predicate",
            predicate,
        );
    }
}
...
mod tests {
    ...
    #[test]
    #[should_panic(
        expected = "expected: <|| panic!(\"hello\")> not to panic\nbut:      it \
    ...
    #[test]
    #[should_panic(
        expected = "expected: <|| panic::panic_any(1)> not to panic\nbut:      it \
        panicked"
    )]
    ...
    #[test]
    #[should_panic(
        expected = "expected: <|| 1> to panic with message <hello>\nbut:      it did \
        not panic"
    )]
    ...
    #[test]
    #[should_panic(
        expected = "expected: <|| panic!(\"hello\")> to panic with message containing \
        <en>\nbut:      it panicked with message <hello>"
    )]
    ...
    #[test]
    #[should_panic(
        expected = "expected: <|| panic::panic_any(1)> to panic with message containing \
        <a>\nbut:      it panicked with non-string message"
    )]
    ...
    #[test]
    #[should_panic(
        expected = "expected: <|| panic!(\"hello!\")> to panic with message matching \
        predicate\nbut:      it panicked with message <hello!>"
    )]
    ...
    #[test]
    #[should_panic(
        expected = "expected: <|| panic::panic_any(1)> to panic with message matching \
        predicate\nbut:      it panicked with non-string message"
    )]
