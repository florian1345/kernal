//! Contains equality-based assertions for [Collection]s with items that implement [Eq] and [Hash],
//! using the additional constraints to optimize runtime. See [CollectionEqHashAssertions] for more
//! details.
...
/// [Eq] and [Hash] in addition to the required [Debug] trait. It offers optimized assertions which
/// benefit from the additional [Hash] constraint in terms of runtime.
///
/// Examples:
///
/// ```
    ...
    C: Collection<'collection>
{
    ...
    /// this function asserts that the tested collection contains at least that number of equal
    /// elements, so `[1, 1, 2]` contains all of `[1, 1]`, but not all of `[1, 1, 1]`.
    ///
    ...
    /// `items` according to [Eq] and [Hash].
    ///
    /// This can be considered a faster alternative to
    ...
    /// tested collection such that matched elements are equal according to [Eq] and [Hash].
    ///
    /// This can be considered a faster alternative to
    ...
    C::Item: Debug + Eq + Hash
{
        ...
        I: IntoIterator<Item = E>
    {
        ...
        check_contains_none_of::<_, _, HashSet<_>>(&self, self.data.iterator(), unexpected_items);
...
#[cfg(test)]
mod tests {
    use crate::{assert_fails, test_contains_all_of, test_contains_exactly_in_any_order, test_contains_none_of};
    use crate::prelude::*;
    ...
    test_contains_none_of!(contains_none_of_using_hash);

    test_contains_exactly_in_any_order!(contains_exactly_in_any_order_using_hash);
}
